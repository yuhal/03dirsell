...
class ShopController extends CommonController {
	...
	public function index() {
		...
			//根据id查询12条商城数据类型(leixin)等于jbzgq,状态等于1;
		...
			//根据id查询10条查询商城书记类型(leixin)等于zsbygq,状态等于1;
		...
			//根据id查询10条查询商城书记类型(leixin)等于zsbxyq,状态等于1;
	...
		//查询商城数据id等于get传来的id,类型(leixin1)等于jbzgq,状态等于1,返回一条;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'leixin' => 'jbzgq' ,
				'zt' => 1
	...
		//查询商城注册用户表UE_ID等于$_SSSION['uid'];返回一条;
		$userData = M ( 'user' )->where ( array (
				'UE_ID' => $_SESSION ['uid']
		) )->find ();
		$this->userData = $userData;
	...
		//查询商城数据id等于get穿来的id,类型等于zsbygq,状态等于1;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'leixin' => 'zsbygq' ,
				'zt' => 1
	...
		//查询商城数据id等于get传来的id,类型等于钻石币协议(zsbxy),状态等于1;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'leixin' => 'zsbxyq' ,
				'zt' => 1
	...
		//查询商场数据id等于get传来的id,user等于$_SESSION['uname'],leixin等于jbzgq,返回一条;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'user' => $_SESSION['uname'] ,
				'leixin' => 'jbzgq' ,
	...
		//查询商场数据id等于get传来的id,user等于$_SESSION['uname'],leixin等于zsbygq,返回一条;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'user' => $_SESSION['uname'] ,
				'leixin' => 'zsbygq' ,
	...
		//查询商城数据ID等于get传来的id,user等于$_SESSION['uname'],leixin等于zsbxyq,返回一条;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'user' => $_SESSION['uname'] ,
				'leixin' => 'zsbxyq' ,
	...
		//删除商城数据ID等于get传来的id,user等于$_SESSION['uname'],leixin等于jbzgq;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'user' => $_SESSION['uname'] ,
				'leixin' => 'jbzgq' ,
	...
			//删除商城数据ID等于get传来的id,user等于$_SESSION['uname'],leixin等于zsbygq;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'user' => $_SESSION['uname'] ,
				'leixin' => 'zsbygq' ,
	...
		//查询商城数据id等于get传来的id,user等于$_SESSION['uname'],类型等于zsbxyq,返回一条;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'user' => $_SESSION['uname'] ,
				'leixin' => 'zsbxyq' ,
	...
		//删除商城数据id等于get传来的id,user等于$_SESSION['uname'],类型等于zsbxyq;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('get.id') ,
				'user' => $_SESSION['uname'] ,
				'leixin' => 'zsbxyq' ,
	...
	public function zsbyg_ljgm() {
		...
			//查询商城数据id等于post的id,类型等于zsbygq,状态等于1,返回一条;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('post.id') ,
				'leixin' => 'zsbygq' ,
				'zt' => 1
		...
			//判断用户的可购份数(lxfs)是否等于0;
		if($caution['lxfs']=='0'){
			$this->error('產品可購份數不足!');	//正则匹配post传来的购买份数,长度1,100,0-9;
		}elseif(! preg_match ( '/^[0-9]{1,100}$/', I('post.gmfs') )){
			$this->error('購買份數輸入有誤!');	//判断用户的可购份数是否小于post传来的购买份数
		}elseif($caution['lxfs']<I('post.gmfs')){
			$this->error('產品可購份數不足!');	//判断钻石币总和是否小于购买份数乘经营项目
		}elseif($user['zsbhe']<I('post.gmfs')*$caution['jyxm']){
			$this->error('餘額不足,不可購買!');	//判断post传来的手机(sj)是否为空
		}elseif(I('post.sj')==''){
			$this->error('手機不能為空!');	//判断post传来的收货人(shr)是否为空
		}elseif(I('post.shr')==''){
			$this->error('收貨人不能為空!');	//判断post传来的收货地址(shdz)是否为空
		}elseif(I('post.shdz')==''){
			$this->error('收貨地址不能為空!');
		}
		
			//设置商城注册用户表UE_account等于$_SESSION['uname']中钻石币总和等于post传来的购买份数乘经营项目;
		M('user')->where(array('UE_account'=>$_SESSION['uname']))-> setDec('zsbhe',I('post.gmfs')*$caution['jyxm']);	//设置商城数据表id等于post传来的id,类型(leixin)等于zsbygq,状态等于1中可购份数等于post传来的uname;
	...
		//查询商城数据表id等于post传来的id,类型(leixing)等于zsbxyq,状态等于1,返回一条;
		$caution = M ( 'shopsj' )->where ( array (
				'id'=> I('post.id') ,
				'leixin' => 'zsbxyq' ,
				'zt' => 1
	...
		//设置商城注册用户表UE_account等于$_SESSION['uname']中zsbhe等于经营项目(jyxm);
		M('user')->where(array('UE_account'=>$_SESSION['uname']))-> setDec('zsbhe',$caution['jyxm']);	//设置商城数据表id等于post传来的id,类型等译zsbygq,状态等于1;
		M ( 'shopsj' )->where ( array (
		'id'=> I('post.id') ,
		'leixin' => 'zsbygq' ,
		'zt' => 1
	...
	public function sjsqclxg() {
		...
		$data['date']=date ( 'Y-m-d H:i:s', time () );
		$data['leixin']='jbzgq';
		//修改商城数据表id等于post传来的id,user等于$_SESSION['uname']中对应字段的数据;
	...
	public function dzpcl() {
		...
			//设置商城注册用户表UE_account等于$_SESSION['uname']中UE_money等于5;
		...
			//设置商城注册用户表UE_account等于$_SESSION['uname']中UE_money等于奖金9(jj);
		$user->where(array('UE_account'=>$_SESSION ['uname']))->setInc('UE_money',$res['jj']);	//查询商城注册用户表UE_accountd等于$_SESSION['uname'],返回一条;
		$userxx=$user->where(array('UE_account'=>$_SESSION ['uname']))->find();
